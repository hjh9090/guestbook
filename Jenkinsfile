...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="neoguli/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/hjh9090/guestbook.git'
            }
        }
        stage('Build') {
            steps {
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'xswzaq7412@naver.com')
